...
// import { Cartographic, CartographicRange } from "@itwin/core-common";
...
describe("RealityServicesClient Normal (#integration)", () => {
  ...
  it("should be able to retrieve reality data properties for every reality data associated with iTwin", async () => {
    ...
    const realityDatas = realityDataResponse.realityDatas as ITwinRealityData[];
  ...
  it("should be able to query using continuationToken", async () => {
    ...
    const realityDatasContinued = realityDataResponseContinuation.realityDatas as ITwinRealityData[];
...
/*
  it("should be able to retrieve reality data properties for every reality data associated with iTwin within an extent", async () => {
    ...
    let maxLongitude: number = +footprint.Coordinates[0].Longitude;
    let maxLatitude: number = +footprint.Coordinates[0].Latitude;
    let minLongitude: number = +footprint.Coordinates[0].Longitude;
    let minLatitude: number = +footprint.Coordinates[0].Latitude;
    for (const entry of footprint.Coordinates) {
      maxLongitude = Math.max(maxLongitude, +entry.Longitude);
      minLongitude = Math.min(minLongitude, +entry.Longitude);
      maxLatitude = Math.max(maxLatitude, +entry.Latitude);
      minLatitude = Math.min(minLatitude, +entry.Latitude);
    }

    const cornerCarto = new Array<Cartographic>();
    cornerCarto[0] = Cartographic.fromDegrees({ longitude: minLongitude, latitude: minLatitude, height: 0.0 });
    cornerCarto[1] = Cartographic.fromDegrees({ longitude: minLongitude, latitude: maxLatitude, height: 0.0 });
    cornerCarto[2] = Cartographic.fromDegrees({ longitude: maxLongitude, latitude: maxLatitude, height: 0.0 });
    cornerCarto[3] = Cartographic.fromDegrees({ longitude: maxLongitude, latitude: minLatitude, height: 0.0 });
